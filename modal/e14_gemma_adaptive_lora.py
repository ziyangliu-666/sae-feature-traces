"""Recipe 2 (Round 6): one Gemma adaptive LoRA point — cross-backbone
adaptive-frontier coverage.

The Round-5 reviewer flagged that the adaptive-attacker frontier is
Qwen-only. This is the minimum-cost experiment that closes that gap:
  - Target: google/gemma-2-2b at layer 12 + gemma-scope-2b-pt-res-canonical
  - Substitute: google/gemma-2-2b-it (closest same-family attacker)
  - Adaptation: rank-64 LoRA on seven projection modules, joint-phi
    linear+bias map initialised from public-corpus least-squares then
    fine-tuned. 1500 steps, lambda_util=0 (Stage A, pure-probe).

We report a single number: best adaptive Gemma joint_z / tau_Gemma,
plus utility hit (Pile ppx ratio). Reviewer-sway target: joint_z still
>= 1.5 x tau even with library-aware adversary.

Modal L4, ~0.2-0.5 GPU-h (~$0.15-0.40).

Reuses the Gemma pilot's probe library + sigma-calibration from
results/e12_gemma_pilot.json so the threshold is comparable.
"""
import json
from pathlib import Path
import modal

app = modal.App("e14-gemma-adaptive-lora")
GPU = "L4"
...
PILOT_JSON = LOCAL_ROOT / "results" / "e12_gemma_pilot.json"
...
def run(pilot_blob: dict) -> dict:
    ...
    print(f"[load] target Gemma-2-2B in {time.time()-t0:.1f}s vram={torch.cuda.memory_allocated()/1e9:.2f}GB")
    ...
    tgt.model.layers[LAYER].register_forward_hook(hook_tgt)
    ...
    def build_substitute(rank=64, seed=0):
        ...
        h_handle = sub.base_model.model.model.layers[LAYER].register_forward_hook(hook_sub)
        return sub, h_handle

    def fit_initial_phi(sub_model):
        """Pre-fit phi (linear + bias) via LSQ on ~1000 Pile pairs."""
        sub_model.eval()
        X_s, X_t = [], []
        with torch.no_grad():
    ...
    print("\n=== Build Gemma substitute (rank=64) ===")
...
def main():
    ...
    out_path = LOCAL_ROOT / "results" / "e14_gemma_adaptive_lora.json"
    out_path.write_text(json.dumps(result, indent=2))
    md = result["metadata"]
    print(f"\n[save] {out_path}")
    print(f"\n=== Gemma Stage A summary (tau_99_synth = {md['honest_tau_99_synthetic']:.2f}) ===")
