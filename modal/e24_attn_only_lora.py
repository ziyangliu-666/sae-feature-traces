...
Output: results/e16_whitebox_jointz_{backbone}.json
"""
import json
import argparse
...
app = modal.App("e24-attn-only-lora")
GPU = "A10G"
...
def main(backbone: str = "qwen3", n_steps: int = 3000, lora_rank: int = 64,
         ...
         seed_offset: int = 0):
    cfg = BACKBONES[backbone]
    lib_path = LOCAL_ROOT / "results" / cfg["lib"]
    sig_path = LOCAL_ROOT / "results" / cfg["sigma"]
    ...
    out = LOCAL_ROOT / "results" / f"e24_attn_only_{backbone}{suffix}.json"
