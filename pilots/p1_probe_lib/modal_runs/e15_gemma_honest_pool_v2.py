"""E15-v2: Gemma-2-2B honest pool with RECALIBRATED sigma.

v1 (e15_gemma_honest_pool.py) reused the 8-config E12 sigma and got
tau_real_99=57.99 because many per-feature sigmas were under-estimated
(Gemma has long-tailed feature magnitudes; see
logs/e12_gemma_pilot.json:sigma_calibration). v2 does two phases in a
single Modal session:

  Phase A (sigma): 32 configs per probe (2 dtypes x 2 kernels x 4
    positions x 2 seeds, seeds 50-51 dedicated to calibration).
  Phase B (honest pool): 64 configs per probe (2 x 2 x 4 x 4, seeds
    200-203 disjoint from Phase A).

Reuses Gemma probe library from e12_gemma_pilot.json. Writes a fresh
sigma_calibration_gemma2_2b_L12.json AND recipe1_gemma2_honest_pool_v2.json.

Est. wall on L4: ~15-25 min -> ~$0.3.
"""
import json
import modal
from pathlib import Path

app = modal.App("e15-gemma-honest-pool-v2")
GPU = "L4"
...
E12_JSON = LOCAL_ROOT / "logs" / "e12_gemma_pilot.json"
SIG_OUT = LOCAL_ROOT / "logs" / "sigma_calibration_gemma2_2b_L12.json"
POOL_OUT = LOCAL_ROOT / "logs" / "recipe1_gemma2_honest_pool_v2.json"
...
def run(probes: list) -> dict:
    ...
    POSITIONS = [0, 1, 2, 3]
    CAL_SEEDS = [50, 51]  # disjoint from honest-pool seeds
    ...
    sig_by_probe = {}
    for p in probes:
        S = np.stack(sig_snaps[p["probe_id"]])  # [n_cal, top_k]
    ...
    # Persist sigma immediately (so we can reuse if Phase B errors)
    os.makedirs("/cache/e15v2_out", exist_ok=True)
    with open("/cache/e15v2_out/sigma_calibration_gemma2_2b_L12.json", "w") as f:
        json.dump(sigma_out, f, indent=2)

    # Diagnostic on sigma distribution
    ...
    print(f"\n[phase B] honest pool: {n_pool} configs x {len(probes)} probes")
    tB = time.time()
    configs = []
    per_probe_maha_all = []
    pool_idx = 0
    ...
    print(f"\n=== Gemma honest pool v2 summary (n={len(joints)}) ===")
    ...
    print(f"tau_real_99 = {tau_real_99:.4f}, CP 95% upper at 0/64: {cp_at_zero_n64:.4f}")

    out = {
        "metadata": {
            "experiment": "Recipe1: Gemma-2-2B honest pool v2 (recalibrated sigma)",
            "target": TARGET_MODEL,
        ...
        "exceedances": {
            "at_tau_real_p99": {"count": exceed_real_99, "n": 64, "cp95_upper_ci": cp_at_real_99},
        },
    ...
    with open("/cache/e15v2_out/recipe1_gemma2_honest_pool_v2.json", "w") as f:
...
def main():
    ...
    probes = e12["probes"]
    print(f"reusing {len(probes)} Gemma probes from e12_gemma_pilot.json")
    ...
    print(f"wall: {out['pool']['metadata']['wall_s']:.0f}s  "
          f"tau_real_99={out['pool']['statistics']['p99_empirical']:.4f}")
